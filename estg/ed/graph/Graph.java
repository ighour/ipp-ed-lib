...
public class Graph<T> extends BaseGraph<T> implements GraphADT<T> {
    ...
    /**
     * Removes a single vertex with the given value from this graph. Throws
    ...
    /**
     * Inserts an edge between two vertices of this graph. Edge is directional,
     * so addEdge(A,B) is different from addEdge(B,A). Throws
    ...
    /**
     * Removes an edge between two vertices of this graph. Edge is directional,
     * so removeEdge(A,B) is different from removeEdge(B,A). Throws
     * ElementNotFoundException if vertex is not found at graph. Uses
     * changeEdge() method to handle edge update.
     *
     * @param vertex1 the first vertex
